...
void ParallelWorker(long ** A, size_t num_rows_a, size_t num_cols_a,
                              long ** B, size_t num_rows_b, size_t num_cols_b,
							  long ** C, size_t startRowC, size_t endRowC)
{
    for (size_t i = startRowC; i < endRowC; ++i)
    {
        for (size_t j = 0; j < num_cols_b; ++j)
        {
							  ...
							  long ** C) 
{
    ...
    size_t basketSize = ceil((float)num_rows_a/numWorkers);

    std::thread workers[numWorkers];
    for(size_t i = 0; i < numWorkers; ++i)
    {
        size_t startRowC = std::min(i*basketSize, num_rows_a);
        size_t endRowC = std::min((i+1)* basketSize, num_rows_a);
        workers[i] = std::thread(ParallelWorker, A, num_rows_a, num_cols_a,
